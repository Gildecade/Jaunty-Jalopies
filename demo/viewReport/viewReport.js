...
router.post("/grossCustomerIncome", async(req, res) => {
    ...
    ON t1.customer_id = t2.id) as t6
    union
    (SELECT
    t2.id as customer_id,
    t1.start_date as date,
    t1.labor_charge+t3.parts_cost as income
    FROM Repair as t3
    LEFT JOIN Customer as t4
    ON t3.customer_id = t2.id
    ) as t7
    GROUP BY customer_id
    ORDER BY gross_income DESC, last_date DESC
    LIMIT 15;`
...
// TODO: view gross customer income more details -- vehicle sale 
...
// TODO: view gross customer income more details -- vehicle repairs
router.post("/grossCustomerIncome/repairs", async(req, res) => {
    ...
    t1.larbor_charge as larbor_cost,
    t1.parts_cost + t1.larbor_charge as total_cost,
    concat(t2.first_name, " ", t2.last_name) as service_writer_name
    FROM Repair t1
    INNER JOIN User t2
    ON t1.service_writer_username = t2.username
    WHERE t1.customer_id = '${customer_id}'
    ORDER BY start_date DESC, end_date DESC NULLS FIRST, VIN ASC;`;
...
// TODO: view repair report
router.post("/repairReport", async(req, res) => {
    let sql = `SELECT t3.name as manufacturer,
    count(1) as count_of_repairs,
    ...
    INNER JOIN Vehicle t2
    ON t1.vin = t2.vin
    RIGHT JOIN Manufacturer t3
    ON t2.manufacturer = t3.name
...
// TODO: view repair report -- the repair stats of vehicle type of selected manufacturer
router.post("/repairReport/type", async(req, res) => {
    ...
    HAVING t2.manufacturer = '${manufacturer}'
...
// TODO: view repair report -- the repair stats of model of selected manufacturer
...
// TODO: view average time in inventory
router.post("/averageTime", async(req, res) => {
    let sql = `SELECT t1.vehicle_type as vehicle_type,
    avg(datediff(Day, t2.purchase_date, t1.added_date)) as average_time
    FROM Vehicle t1
    INNER JOIN Sale t2
    ON t1.vin = t2.vin
    GROUP BY vehicle_type;`;
...
// TODO: view monthly sales of selected month
